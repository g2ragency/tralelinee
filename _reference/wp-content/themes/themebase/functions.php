...
function wp_bootstrap_starter_setup() {
	...
	register_nav_menus( array(
		'primary' => esc_html__( 'Header Alto', 'altera-starter' ),
		'secondary' => esc_html__( 'Header Basso', 'altera-starter' ),
	) );
...
// =============================
// Abilita upload SVG (con controllo)
// =============================
function altera_g_theme_mime_types($mimes) {
    $mimes['svg'] = 'image/svg+xml';
    return $mimes;
}
add_filter('upload_mimes', 'altera_g_theme_mime_types');

function altera_g_theme_svg_sanitization($file, $file_name, $mimes) {
    $filetype = wp_check_filetype($file_name);
    if ($filetype['ext'] === 'svg' && !current_user_can('manage_options')) {
        $file['error'] = 'Non hai i permessi per caricare file SVG.';
    }
    return $file;
}
add_filter('wp_check_filetype_and_ext', 'altera_g_theme_svg_sanitization', 10, 3);