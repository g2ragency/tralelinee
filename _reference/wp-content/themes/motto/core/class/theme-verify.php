    ...
    class WGL_Theme_Verify
    {
        ...
        */
        ...
        private function hooks(){
            ...
            add_action( 'wp_ajax_reset_purchase', array( $this, 'reset_purchase' ) );
            add_action( 'wp_ajax_nopriv_reset_purchase', array( $this, 'reset_purchase' ));
            ...
            <div class="notice notice-error is-dismissible">
                <p>
                    <?php echo wp_get_theme()->get('Name');
                        echo sprintf( esc_html__( ' Theme is not activated! Please activate your copy of the %s Theme.', 'motto'), wp_get_theme()->get('Name') );
                        ?>
                </p>
                <p>
                    <strong class="activate_error"><?php esc_html_e( 'Please activate the theme!', 'motto' ); ?></strong> -
                    <a href="<?php echo esc_url(( $page_purchase )); ?>">
                        <?php esc_html_e( 'Activate Now','motto' ); ?>
                    </a>
                </p>
            </div>
        ...
        public function wgl_extensions_requirements(){
            ...
            <div class="notice notice-error is-dismissible">
                <p>
                    <?php
                        echo esc_html__( 'WGL Extensions plugin is not activated! Please install and activate it first.', 'motto');
                    ?>
                </p>
            </div>
            ...
            <div class="notice notice-warning is-dismissible">
                <p class="support-until_text">
                    <?php
                        ...
                        if($now > $until_date){
                            echo sprintf( wp_kses( __( '<strong>Your support period expired. <a href="%s">You can prolong support</a> services as you need.</strong>', 'motto' ), $allowed_html), esc_url($item_url));
                        }elseif($now < $until_date && $interval->days < 30){
                            echo sprintf( wp_kses( __( '<strong>Your support period will be expired after %d days. <a href="%s">You can prolong support</a> services as you need.</strong>', 'motto' ), $allowed_html), $interval->days, esc_url($item_url));
                        }
                ...
                <p>
                    ...
                    ?>
                    <a href="<?php echo esc_url($remind_url); ?>" class="button button-primary"><?php echo esc_html__( 'Remind me later', 'motto' ); ?></a>&nbsp;
                    <a href="<?php echo esc_url($delete_url); ?>" class="button button-primary"><?php echo esc_html__( 'Hide and disable further reminders', 'motto' ); ?></a>
                </p>
            </div>
        ...
        public function purchase_activation(){
            ...
            if ( ! isset( $_POST['email'] ) ||  ! isset( $_POST['purchase_code'] ) || ! isset( $_POST['security'] ) || ! wp_verify_nonce( $_POST['security'], 'purchase-activation' ) ) {
                $output['error'] = 1;
                $output['message'] = esc_html__( 'Please enter a valid field', 'motto' );
                ...
                if( ! is_email( $email ) ){
                    $output['error'] = 1;
                    $output['message'] = esc_html__( 'Please enter a valid email address.', 'motto' );
                    echo json_encode( $output );
                    wp_die();
                }
                ...
                if(empty($purchase)){
                    $output['error'] = 1;
                    $output['message'] = esc_html__( 'Purchase code is empty ', 'motto' );
                    echo json_encode( $output );
                    wp_die();
                }
                ...
                if( $return !== false ){
                    ...
                    if(isset($result['success']) && !empty($result['success'])){
                        ...
                        $output['message'] =  esc_html__( 'Thank you, your license has been validated', 'motto' );
                        ...
                        echo json_encode( $output );
                    }elseif(isset($result['status']) && 678 === $result['status']){
                        $output['message']  = '<div class="register_purchase">';
                        $output['message'] .= '<span class="register_purchase--title">'.esc_html__( 'This purchase code has been registered.', 'motto' ).'</span><br/>';
                        $output['message'] .= esc_html__( 'Please go to your previous working environment and deactivate the purchase code to use it again (WP dashboard -> WebGeniusLab -> Activate Theme -> click on the button "Deactivate" ).', 'motto' ).'<br/><br />';
                        $output['message'] .='<a class="register_purchase--form" href="#">Lost access to your previous site?</a>';
                        $output['message'] .='</div>';
                        $output['success'] = '';
                        $output['error'] = 1;
                        $output['status'] = 678;
        ...
        public function reset_purchase(){
            $output = array( 'success'   => 0, 'message'   => '', 'error'     => '');

            if ( ! isset( $_POST['email'] ) || ! isset( $_POST['accout'] ) ||  ! isset( $_POST['purchase_code'] ) || ! isset( $_POST['security'] ) || ! wp_verify_nonce( $_POST['security'], 'purchase-reset-activation' ) ) {
                $output['error'] = 1;
                $output['message'] = esc_html__( 'Please enter a valid field', 'motto' );
                echo json_encode( $output );
                wp_die();
                ...
                if(empty($accout)){
                    $output['error'] = 1;
                    $output['message'] = esc_html__( 'Your Envato Account is empty ', 'motto' );
                    echo json_encode( $output );
                    wp_die();
                }

                $response = get_transient('wgl_purchase_cache_' . $purchase);
                if (false === $response) {
                    $return = self::reset_activation($email, $purchase, $accout);
                    if( $return !== false ){
                        $result = json_decode( $return['body'], true );
                        ...
                        if(isset($result['success']) && !empty($result['success'])){
                            $output['success'] = 1;
                            $output['error'] = 0;
                            $output['message'] =  esc_html__( 'Your Request Has been Sent', 'motto' );
                            set_transient('wgl_purchase_cache_'.$purchase, true, DAY_IN_SECONDS);
                            echo json_encode( $output );
                        }else{
                            $output['success'] = '';
                            $output['message'] = $result['message'];
                    ...
                    $output['success'] = '';
                    $output['message'] = esc_html__( 'The request has been sent before. You will get an email when it is reviewed.', 'motto' );
                    $output['error'] = 1;
                    echo json_encode( $output );
                }
            }
        ...
        public static function reset_activation($email, $purchase, $accout){

            $url = WGL_Theme_Verify::get_instance()->api. 'reset_activation';
