...
if (!class_exists( 'WGL_Mega_Menu_Walker')) {
     ...
     * @since 1.0.0
     */
    class WGL_Mega_Menu_Walker extends Walker_Nav_Menu
    {
        /**
         * @since 1.0.0
         */
        ...
        {
            return $depth === 0 && !empty($this->wgl_megamenu_enable) && 'disable' !== $this->wgl_megamenu_enable;
        }
        ...
        public function start_el(
            ...
            $marker_active = WGL_Framework::get_option('marker_active');
            ...
            if (0 === $depth) {
                ...
                if ($this->wgl_megamenu_enable !== '') {
                    $array = ['e_builder', 'width_e', 'pos', 'columns', 'posts_count', 'min_height', 'width', 'padding_left', 'padding_right', 'hide_headings', 'background_image', 'background_repeat', 'background_pos_x', 'background_pos_y'];
            ...
            if ($this->check_mega_menu_activate($depth)) {
                ...
                if ('links' === $this->wgl_megamenu_enable) {
                    $columns = !empty($this->wgl_megamenu_columns) ? $this->wgl_megamenu_columns :  1;
                    $class_names .= ' mega-menu-links mega-columns-'.$columns.'col ';
                }elseif('elementor' === $this->wgl_megamenu_enable){
                    $class_names .= ' mega-menu-e';
                    $position = get_post_meta( $item->ID, 'wgl_megamenu_pos', true );
            ...
            if (
                $depth === 1
                && 'links' === $this->wgl_megamenu_enable
            ...
            $item_output .= $marker_active == '1' ? '<span class="menu-item_dots"></span></span>' : '</span>';
        ...
        {
            if( $this->check_mega_menu_activate($depth) ){
                if( 'elementor' === $this->wgl_megamenu_enable && class_exists( 'Elementor\Plugin' ) && class_exists('\WGL_Extensions\Includes\WGL_Loop_Settings') ){
                    $position = get_post_meta( $item->ID, 'wgl_megamenu_pos', true );
                    $class = "wgl-mega-menu-position-".esc_attr($position);

                    $style = '';
                    if (!empty($this->wgl_megamenu_width_e)) {
                        $style .= "width:".esc_attr((int) $this->wgl_megamenu_width_e)."px;";
                        $style = " style='".$style."'";
                    }
                    $output .= "<div class='wgl-mega-menu mega-menu-container sub-menu wgl-e-container ".$class."'".$style.">";

                        $post_slug = get_post_meta( $item->ID, 'wgl_megamenu_e_builder', true);
                        ...
                        if ( $templates_query->have_posts() ) {
                            ...
                            $output .= \Elementor\Plugin::$instance->frontend->get_builder_content( $page_id, $laod_style );
                        }else{
                            $output .= esc_html__( 'No content found', 'motto' );
                        }
                    $output .= "</div>";
                }
            }
    ...
    /*-----------------------------------------------------------------------------------*/
    add_action( 'wp_nav_menu_item_custom_fields', 'motto_add_megamenu_fields', 10, 4 );
    function motto_add_megamenu_fields( $item_id, $item, $depth, $args )
    {
...
?>
        <p class="description description-wide field-megamenu-submenu-pos col-6">
            <label for="edit-menu-item-megamenu-submenu-pos-<?php echo esc_attr( $item_id ) ?>">
			    <?php esc_html_e( 'Submenu Position', 'motto' ); ?>
                <select id="edit-menu-item-megamenu-submenu-pos-<?php echo esc_attr( $item_id ) ?>" class="widefat code edit-menu-item-megamenu-submenu-pos" name="menu-item-wgl-megamenu-submenu-pos[<?php echo esc_attr( $item_id ) ?>]">
                    <option value="default" <?php selected( $item->wgl_megamenu_submenu_pos, 'default' ); ?>><?php esc_html_e( 'Default', 'motto' ); ?></option>
                    <option value="left" <?php selected( $item->wgl_megamenu_submenu_pos, 'left' ); ?>><?php esc_html_e( 'Left', 'motto' ); ?></option>
                    <option value="right" <?php selected( $item->wgl_megamenu_submenu_pos, 'right' ); ?>><?php esc_html_e( 'Right', 'motto' ); ?></option>
                </select>
            </label>
        </p>
        ...
        <div class="margin-t-20">
            <strong><?php esc_html_e( 'Motto Mega Menu Settings:', 'motto' ); ?></strong>
            <em><?php esc_html_e( '(Only for Main Menu)', 'motto' ); ?></em>
        </div>
        <div class="clear"></div>

        <div class='wgl_accordion_wrapper collapsible close widget_class'>
            <div class='wgl_accordion_heading'>
                <span class='wgl_accordion_title'><?php esc_html_e( 'WGL Mega Menu Settings', 'motto' ); ?></span>
                <span class='wgl_accordion_button'></span>
            </div>
        <div class='wgl_accordion_body'>
            <div class="wgl-mega-menu_wrapper megamenu-lib-ext">
                <p class="description description-wide field-megamenu-enable">
                    <label for="edit-menu-item-megamenu-enable-<?php echo esc_attr( $item_id ) ?>">
                        <?php
                        esc_html_e( 'Enable The WGL Mega Menu?', 'motto' );
                        echo '<select',
                            ...
                            '>',
                            '<option value="">', esc_attr__('Disable', 'motto'), '</option>',
                            '<option value="elementor" ', selected( $item->wgl_megamenu_enable, 'elementor' ), '>', esc_html__('Elementor Builder', 'motto'), '</option>',
                            '<option value="links" ', selected( $item->wgl_megamenu_enable, 'links' ), '>', esc_html__('WP links', 'motto'), '</option>',
                        '</select>';
                        ?>
                    </label>
                </p>
                <?php
                if(class_exists('\WGL_Extensions\Includes\WGL_Loop_Settings')){
                    $query_args = [
                        'post_type' => 'elementor_library',
                        'post_status' => 'publish',
                        'posts_per_page' => -1,
                ...
                ?>
                <p class="description description-wide field-megamenu-e-builder">
                    <label for="edit-menu-item-megamenu-e-builder-<?php echo esc_attr( $item_id ) ?>">
                        <?php esc_html_e( 'Elementor Template:', 'motto' ); ?>
                        <?php
                            if($templates){
                        ?>
                        <select id="edit-menu-item-megamenu-e-builder-<?php echo esc_attr( $item_id ) ?>" class="widefat code edit-menu-item-megamenu-e-builder" name="menu-item-wgl-megamenu-e-builder[<?php echo esc_attr( $item_id ) ?>]">
                            <?php
                                foreach($templates as $slug => $title):
                            ?>
                                <option value="<?php echo esc_attr($slug)?>" <?php selected( $item->wgl_megamenu_e_builder, $slug ); ?>><?php echo esc_html($title)?></option>
                            <?php
                                endforeach;
                            ?>
                        </select>
                        <?php
                                ...
                                echo '<span class="sp-block">'.esc_html__('Nothing Found.', 'motto');
                                echo '<a href="'.esc_url(admin_url('edit.php?post_type=elementor_library&tabs_group=library&elementor_library_type=wgl-mega-menu')).'"> '.esc_html__('Сreate a megamenu template', 'motto').'</a>';
                                echo '</span>';
                            }
                        ?>
                    </label>
                ...
                ?>
                <p class="description description-wide field-megamenu-pos col-6">
                    <label for="edit-menu-item-megamenu-pos-<?php echo esc_attr( $item_id ) ?>">
                        <?php esc_html_e( 'Mega Menu Position', 'motto' ); ?>
                        <select id="edit-menu-item-megamenu-pos-<?php echo esc_attr( $item_id ) ?>" class="widefat code edit-menu-item-megamenu-pos" name="menu-item-wgl-megamenu-pos[<?php echo esc_attr( $item_id ) ?>]">
                            <option value="default" <?php selected( $item->wgl_megamenu_pos, 'default' ); ?>><?php esc_html_e( 'Default', 'motto' ); ?></option>
                            <option value="left" <?php selected( $item->wgl_megamenu_pos, 'left' ); ?>><?php esc_html_e( 'Left', 'motto' ); ?></option>
                            <option value="right" <?php selected( $item->wgl_megamenu_pos, 'right' ); ?>><?php esc_html_e( 'Right', 'motto' ); ?></option>
                            <option value="center" <?php selected( $item->wgl_megamenu_pos, 'center' ); ?>><?php esc_html_e( 'Center', 'motto' ); ?></option>
                            <option value="left-side" <?php selected( $item->wgl_megamenu_pos, 'left-side' ); ?>><?php esc_html_e( 'From Left Side', 'motto' ); ?></option>
                        </select>
                    </label>
                </p>
                <p class="description description-wide field-megamenu-width-e col-6">
                    <label for="edit-menu-item-megamenu-width-e-<?php echo esc_attr( $item_id ) ?>">
                        <?php esc_html_e( 'Template Width', 'motto' );
                        ?>
                        <input type="text" id="edit-menu-item-megamenu-width-e-<?php echo esc_attr( $item_id ) ?>"  class="input-sortable widefat code edit-menu-item-custom" name="menu-item-wgl-megamenu-width-e[<?php echo esc_attr( $item_id ) ?>]" value="<?php echo esc_attr($item->wgl_megamenu_width_e); ?>">
                    </label>
                </p>
                <p class="description description-wide field-megamenu-columns">
                    <label for="edit-menu-item-megamenu-columns-<?php echo esc_attr( $item_id ) ?>">
                        <?php esc_html_e( 'Number of Mega Menu Columns', 'motto' ); ?>
                ...
                <p class="description description-wide field-megamenu-background-image col-6">
                    <label for="edit-menu-item-megamenu-background-image-<?php echo esc_attr( $item_id ) ?>">
                        <?php esc_html_e( 'Background Image', 'motto' );?>
                        <input type="text" class="motto_media_url widefat code edit-menu-item-megamenu-background-image" name="menu-item-wgl-megamenu-background-image[<?php echo esc_attr( $item_id ) ?>]" id="edit-menu-item-megamenu-background-image-<?php echo esc_attr( $item_id ) ?>" value="<?php echo esc_attr($item->wgl_megamenu_background_image); ?>">
                    </label>
                    <a href="#" class="button motto_media_upload wgl_extensions_media_upload"><?php esc_html_e('Upload', 'motto'); ?></a>
                </p>
                ...
                <p class="description description-wide field-megamenu-background-repeat col-6">
                    <label for="edit-menu-item-megamenu-background-repeat-<?php echo esc_attr( $item_id ) ?>">
                        <?php esc_html_e( 'Background Repeat', 'motto' ); ?>
                        <select id="edit-menu-item-megamenu-background-repeat-<?php echo esc_attr( $item_id ) ?>" class="widefat code edit-menu-item-megamenu-background-repeat" name="menu-item-wgl-megamenu-background-repeat[<?php echo esc_attr( $item_id ) ?>]">
                            <option value="no-repeat" <?php selected( $item->wgl_megamenu_background_repeat, 'no-repeat' ); ?>><?php esc_html_e( 'No Repeat', 'motto' ); ?></option>
                            <option value="repeat" <?php selected( $item->wgl_megamenu_background_repeat, 'repeat' ); ?>><?php esc_html_e( 'Repeat', 'motto' ); ?></option>
                            <option value="repeat-x" <?php selected( $item->wgl_megamenu_background_repeat, 'repeat-x' ); ?>><?php esc_html_e( 'Repeat X', 'motto' ); ?></option>
                            <option value="repeat-y" <?php selected( $item->wgl_megamenu_background_repeat, 'repeat-y' ); ?>><?php esc_html_e( 'Repeat Y', 'motto' ); ?></option>
                        </select>
                    </label>
                </p>
                <div class="clear"></div>
                <p class="description description-wide field-megamenu-background-pos-x col-6">
                    <label for="edit-menu-item-megamenu-background-pos-x-<?php echo esc_attr( $item_id ) ?>">
                        <?php esc_html_e( 'Background Position X', 'motto' ); ?>
                        <select id="edit-menu-item-megamenu-background-pos-x-<?php echo esc_attr( $item_id ) ?>" class="widefat code edit-menu-item-megamenu-background-pos-x" name="menu-item-wgl-megamenu-background-pos-x[<?php echo esc_attr( $item_id ) ?>]">
                            <option value="right" <?php selected( $item->wgl_megamenu_background_pos_x, 'right' ); ?>><?php esc_html_e( 'Right', 'motto' ); ?></option>
                            <option value="center" <?php selected( $item->wgl_megamenu_background_pos_x, 'center' ); ?>><?php esc_html_e( 'Center', 'motto' ); ?></option>
                            <option value="left" <?php selected( $item->wgl_megamenu_background_pos_x, 'left' ); ?>><?php esc_html_e( 'Left', 'motto' ); ?></option>
                        </select>
                    </label>
                </p>
                ...
                <p class="description description-wide field-megamenu-background-pos-y col-6">
                    <label for="edit-menu-item-megamenu-background-pos-y-<?php echo esc_attr( $item_id ) ?>">
                        <?php esc_html_e( 'Background Position Y', 'motto' ); ?>
                        <select id="edit-menu-item-megamenu-background-pos-y-<?php echo esc_attr( $item_id ) ?>" class="widefat code edit-menu-item-megamenu-background-pos-y" name="menu-item-wgl-megamenu-background-pos-y[<?php echo esc_attr( $item_id ) ?>]">
                            <option value="top" <?php selected( $item->wgl_megamenu_background_pos_y, 'top' ); ?>><?php esc_html_e( 'Top', 'motto' ); ?></option>
                            <option value="center" <?php selected( $item->wgl_megamenu_background_pos_y, 'center' ); ?>><?php esc_html_e( 'Center', 'motto' ); ?></option>
                            <option value="bottom" <?php selected( $item->wgl_megamenu_background_pos_y, 'bottom' ); ?>><?php esc_html_e( 'Bottom', 'motto' ); ?></option>
                        </select>
                    </label>
                </p>
                <div class="clear"></div>
                <p class="description description-wide field-megamenu-min-height col-6">
                    <label for="edit-menu-item-megamenu-min-height-<?php echo esc_attr( $item_id ) ?>">
                        <?php esc_html_e( 'Min Height', 'motto' );
                        ?>
                ...
                <p class="description description-wide field-megamenu-width col-6">
                    <label for="edit-menu-item-megamenu-width-<?php echo esc_attr( $item_id ) ?>">
                        <?php esc_html_e( 'Max Width', 'motto' );
                        ?>
                 ...
                 <p class="description description-wide field-megamenu-padding-left col-6">
                    <label for="edit-menu-item-megamenu-padding-left-<?php echo esc_attr( $item_id ) ?>">
                        <?php esc_html_e( 'Padding Left', 'motto' );
                        ?>
                ...
                <p class="description description-wide field-megamenu-padding-right col-6">
                    <label for="edit-menu-item-megamenu-padding-right-<?php echo esc_attr( $item_id ) ?>">
                        <?php esc_html_e( 'Padding Right', 'motto' );
                        ?>
                ...
                <p class="description description-wide field-megamenu-hide-headings">
                    <label for="edit-menu-item-megamenu-hide-headings-<?php echo esc_attr( $item_id ) ?>">
                        <?php esc_html_e( 'Hide Mega Menu Headings?', 'motto' );?>
    ...
    add_action('wp_update_nav_menu_item', 'motto_custom_nav_update', 10, 3);
    function motto_custom_nav_update( $menu_id, $menu_item_db_id, $menu_item_data = [] )
    {
        $fields = motto_mega_menu_fields();
    ...
    if (!function_exists('motto_mega_menu_fields')) {

        function motto_mega_menu_fields()
        {
            return [
	            'menu-item-wgl-megamenu-submenu-pos',
                'menu-item-wgl-megamenu-enable',
                'menu-item-wgl-megamenu-e-builder',
                'menu-item-wgl-megamenu-pos',
                ...
                'menu-item-wgl-megamenu-width-e',
    ...
    add_filter( 'wp_edit_nav_menu_walker', 'motto_custom_nav_edit_walker', 10, 2 );
    function motto_custom_nav_edit_walker($walker,$menu_id) {
        return 'Motto_Mega_Menu_Edit_Walker';
    }
     ...
     * @see Walker_Nav_Menu
     */
    class Motto_Mega_Menu_Edit_Walker extends Walker_Nav_Menu
    {
        /**
        ...
        public function start_el( &$output, $item, $depth = 0, $args = [], $id = 0 )
        {
            ...
            if ( ! empty( $item->_invalid ) ) {
                ...
                $title = sprintf( esc_html__( '%s (Invalid)', 'motto' ), $item->title );
            } elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
                $classes[] = 'pending';
                /* translators: %s: title of menu item in draft status */
                $title = sprintf( esc_html__('%s (Pending)', 'motto'), $item->title );
            }
                ...
                <div class="menu-item-bar">
                    <div class="menu-item-handle">
                        <label class="item-title" for="menu-item-checkbox-<?php echo esc_attr($item_id); ?>">
                            ...
                            <span class="is-submenu <?php echo esc_attr( $submenu_text ); ?>"><?php esc_html_e( 'sub item', 'motto' ); ?></span>
                        </label>
                        <span class="item-controls">
                            ...
                            <span class="item-order hide-if-js">
                                ...
                                ?>" class="item-move-up" aria-label="<?php esc_attr_e( 'Move up', 'motto' ) ?>">&#8593;</a>
                                |
                                <a href="<?php
                                    echo esc_url(wp_nonce_url(
                                        add_query_arg(
                                ...
                                ?>" class="item-move-down" aria-label="<?php esc_attr_e( 'Move down', 'motto'  ) ?>">&#8595;</a>
                            </span>
                            ...
                            ?>" aria-label="<?php esc_attr_e( 'Edit menu item', 'motto'  ); ?>"><span class="screen-reader-text"><?php esc_html_e( 'Edit', 'motto' ); ?></span></a>
                        </span>
                    </div>
                </div>
                ...
                <div class="menu-item-settings wp-clearfix" id="menu-item-settings-<?php echo esc_attr($item_id); ?>">
                    ...
                    <p class="description description-wide">
                        <label for="edit-menu-item-title-<?php echo esc_attr($item_id); ?>">
                            <?php esc_html_e( 'Navigation Label', 'motto' ); ?><br />
                            <input type="text" id="edit-menu-item-title-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-title" name="menu-item-title[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( $item->title ); ?>" />
                        </label>
                    </p>
                    <p class="field-title-attribute field-attr-title description description-wide">
                        <label for="edit-menu-item-attr-title-<?php echo esc_attr($item_id); ?>">
                            <?php esc_html_e( 'Title Attribute', 'motto' ); ?><br />
                    ...
                    <p class="field-link-target description">
                        <label for="edit-menu-item-target-<?php echo esc_attr($item_id); ?>">
                            ...
                            <?php esc_html_e( 'Open link in a new tab', 'motto' ); ?>
                        </label>
                    </p>
                    <p class="field-css-classes description description-thin">
                        <label for="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>">
                            <?php esc_html_e( 'CSS Classes (optional)', 'motto' ); ?><br />
                            <input type="text" id="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-classes" name="menu-item-classes[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( implode(' ', $item->classes ) ); ?>" />
                        </label>
                    </p>
                    <p class="field-xfn description description-thin">
                        <label for="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>">
                            <?php esc_html_e( 'Link Relationship (XFN)', 'motto' ); ?><br />
                            <input type="text" id="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-xfn" name="menu-item-xfn[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( $item->xfn ); ?>" />
                        </label>
                    </p>
                    <p class="field-description description description-wide">
                        <label for="edit-menu-item-description-<?php echo esc_attr($item_id); ?>">
                            <?php esc_html_e( 'Description', 'motto' ); ?><br />
                            <textarea id="edit-menu-item-description-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-description" rows="3" cols="20" name="menu-item-description[<?php echo esc_attr($item_id); ?>]"><?php echo esc_html( $item->description ); // textarea_escaped ?></textarea>
                            <span class="description"><?php esc_html_e('The description will be displayed in the menu if the current theme supports it.', 'motto'); ?></span>
                        </label>
                    </p>
                    <?php
                        /*-----------------------------------------------------------------------------------*/
                    ...
                    <fieldset class="field-move hide-if-no-js description description-wide">
                        <span class="field-move-visual-label" aria-hidden="true"><?php esc_html_e( 'Move', 'motto' ); ?></span>
                        <button type="button" class="button-link menus-move menus-move-up" data-dir="up"><?php esc_html_e( 'Up one', 'motto' ); ?></button>
                        <button type="button" class="button-link menus-move menus-move-down" data-dir="down"><?php esc_html_e( 'Down one', 'motto' ); ?></button>
                        <button type="button" class="button-link menus-move menus-move-left" data-dir="left"></button>
                        <button type="button" class="button-link menus-move menus-move-right" data-dir="right"></button>
                        <button type="button" class="button-link menus-move menus-move-top" data-dir="top"><?php esc_html_e( 'To the top', 'motto' ); ?></button>
                    </fieldset>
                    ...
                    <div class="menu-item-actions description-wide submitbox">
                        <?php if ( 'custom' != $item->type && $original_title !== false ) : ?>
                            <p class="link-to-original">
                                ...
                                printf( wp_kses( __('Original: %s', 'motto'), $allowed_html ), '<a href="' . esc_url( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
                            </p>
                        <?php endif; ?>
                        ...
                        )); ?>"><?php esc_html_e( 'Remove', 'motto' ); ?></a> <span class="meta-sep hide-if-no-js"> | </span> <a class="item-cancel submitcancel hide-if-no-js" id="cancel-<?php echo esc_attr($item_id); ?>" href="<?php echo esc_url( add_query_arg( [ 'edit-menu-item' => $item_id, 'cancel' => time() ], admin_url( 'nav-menus.php' ) ) );
                            ?>#menu-item-settings-<?php echo esc_attr($item_id); ?>"><?php esc_html_e('Cancel', 'motto'); ?></a>
                    </div>
